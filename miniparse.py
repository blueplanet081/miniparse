...
def miniparse(tm: 'Args', pms: Dict[str, 'Oset'], mode: int = 0) -> List[str]:
    ''' コマンドライン解析用モジュール
    '''
                ...
                if p in pms:
                    ...
                    if pms[p].needParam:    # オプションパラメータ必要
                        if op := tm.getAll():
                            ...
                            raise ParseError(f"missing option parameter of '{p}'")
                        break
                else:
...
if __name__ == "__main__":
    ...
    ret = []
    myArgs = Args(sys.argv[1:])
    try:
        ret = miniparse(myArgs, pm, 1)
